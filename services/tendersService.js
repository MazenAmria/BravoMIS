...
const getClosedTenders = function getClosedTenders(username, callback) {
    db.query(`SELECT * FROM tender WHERE vending_manager_id LIKE '${username}' AND status LIKE 'closed';`, (err, results) => {
        if (err) callback(err, null);
...
const getResolvedTenders = function getResolvedTenders(callback) {
    db.query(`SELECT * FROM tender WHERE status LIKE 'resolved';`, (err, results) => {
        if (err) callback(err, null);
        callback(null, results);
    });
}
